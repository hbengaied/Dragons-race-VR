...
using UnityEngine;
public class PlateformOnlyHorizontal : MonoBehaviour
{
    ...
    // Heave change step
    private float DRAWING_HEAVE_STEP = 0.05f;
    ...
    public void inverseModule()
    {
        ...
        {
            ModuloValue = 0.5f;
            DRAWING_HEAVE_STEP = 0.2f;
        }
        else
        {
            ModuloValue = 2f;
            DRAWING_HEAVE_STEP = 0.05f;
    ...
    void Start()
    {
        ...
        if (m_fsmi.IsLoaded())
        {
            ...
            m_board = GameObject.Find("MyDragon");
    ...
    void FixedUpdate()
    {
        ...
        UpdateValueHeave(ref m_heave, DRAWING_HEAVE_STEP, 0, DRAWING_HEAVE_MAX, ModuloValue);
    }
    ...
    private void UpdateValueHeave(ref float value, float step, float min, float max, float modulo)
    {
        float time = Time.timeSinceLevelLoad;

        // Alterner entre positif et négatif chaque seconde
        if (time % modulo < (modulo/2))
        {
            // Pendant la première seconde, value est positif
            value = Mathf.Clamp(value + step, min, max);
        }
        else
        {
            // Pendant la seconde suivante, value est négatif
            value = Mathf.Clamp(value - step, min, max);
        }
    }
